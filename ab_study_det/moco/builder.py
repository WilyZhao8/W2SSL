...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, mlp_dim=128, T=1.0):
        ...
        self.base_encoder.fc_loc = nn.Linear(2048, 4)
        self.momentum_encoder.fc_loc = nn.Linear(2048, 4)
    ...
    def _batch_gather_ddp2(self, images, permute):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        col1 = torch.cat([images_gather[0:n], images_gather[n:2 * n]], dim=3)
        col2 = torch.cat([images_gather[2 * n:3 * n], images_gather[3 * n:4 * n]], dim=3)
        # col1 = torch.cat([images_gather[4 * n:5 * n], images_gather[5 * n:6 * n]], dim=3)
        # col2 = torch.cat([images_gather[6 * n:7 * n], images_gather[7 * n:8 * n]], dim=3)
        images_gather = torch.cat([col1, col2], dim=2)
    ...
    def forward(self, x1, x2, m):
        ...
        #q1_gather,q11,q12,q13,q14 = self.forward_(q1)
        q1_gather = self.forward_(q1)

        q2 = self.base_encoder(images_gather2)
        #q2_gather,q21,q22,q23,q24 = self.forward_(q2)
        q2_gather = self.forward_(q2)


        # clustering branch
        #label_clu = permute1 % bs_all1
        #q_clu = self.base_encoder.fc_clu(q1_gather)
        #q_clu = nn.functional.normalize(q_clu, dim=1)
        #loss_clu = self.criterion_clu(q_clu, label_clu)


        # location branch
        label_loc = torch.LongTensor([0] * bs_all1 + [1] * bs_all1 + [2] * bs_all1 + [3] * bs_all1).cuda()
        ...
        # # crop branch
        # cq1 = self.predictor(self.base_encoder.fc_crop(q1_gather))
        # #print('cq1',cq1.shape)
        # cq2 = self.predictor(self.base_encoder.fc_crop(q2_gather))
        # with torch.no_grad():  # no gradient
        #     self._update_momentum_encoder(m)  # update the momentum encoder
        ...
        #     ck2 = self.momentum_encoder.fc_crop(self.forward_(self.momentum_encoder(images_gather2)))
        #
        # return loss_clu,loss_loc,1*(self.contrastive_loss(cq1, ck2) + self.contrastive_loss(cq2, ck1))
